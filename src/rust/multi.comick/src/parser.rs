		...
		&& completed.is_empty()
	{
		...
		for data in json {
			if let Ok(data_obj) = data.as_object() {
				let title = data_obj.get("title").as_string()?.read();
				let id = data_obj.get("slug").as_string()?.read();
				let cover = data_obj.get("cover_url").as_string()?.read();
				mangas.push(Manga {
					id,
					cover,
					title,
	...
	for data in json {
		if let Ok(data_obj) = data.as_object() {
			if let Ok(manga_obj) = data_obj.get("md_comics").as_object() {
				...
				let id = manga_obj.get("slug").as_string()?.read();
				let cover = manga_obj.get("cover_url").as_string()?.read();
				if get_lang_code() == "zh-hk" {
					...
					for lang in manga_json.get("langList").as_array()? {
						lang_list.push(lang.as_string()?.read())
					}
					if !lang_list.contains(&get_lang_code()) {
						continue;
					}
				}
				mangas.push(Manga {
...
pub fn parse_manga_details(api_url: String, id: String) -> Result<Manga> {
	let url = format!("{}/comic/{}?tachiyomi=true", api_url, id);
...
pub fn parse_chapter_list(api_url: String, id: String) -> Result<Vec<Chapter>> {
	...
	for chapter in mchapters {
		...
		let title = data_from_json(chapter_obj.clone(), "title");
		let volume = chapter_obj.get("vol").as_float().unwrap_or(-1.0) as f32;
		let hid = data_from_json(chapter_obj.clone(), "hid");
...
pub fn parse_page_list(api_url: String, id: String) -> Result<Vec<Page>> {
	let mut pages: Vec<Page> = Vec::new();
	let url = format!("{}/chapter/{}?tachiyomi=true", api_url, id,);
	let json = Request::new(&url, HttpMethod::Get).json().as_object()?;
