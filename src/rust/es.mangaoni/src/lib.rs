...
static BASE_URL: &str = "https://manga-oni.com/";
...
fn get_manga_listing(listing: Listing, page: i32) -> Result<MangaPageResult> {
	match listing.name.as_str() {
		"Latest" => {
			let url = String::from("https://manga-oni.com/library?order_item=creation&order_dir=desc&filter_by=title&_pg=1&page=") + &page.to_string();
			parser::parse_manga_list(url)
		}
		"Popular" => {
			let url = String::from("https://manga-oni.com/library?order_item=likes_count&order_dir=desc&filter_by=title&_pg=1&page=") + &page.to_string();
			parser::parse_manga_list(url)
		}
