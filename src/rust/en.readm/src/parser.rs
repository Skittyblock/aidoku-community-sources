...
use aidoku::{
    ...
    DeepLink, Filter, FilterType, Manga, MangaContentRating, MangaPageResult, MangaStatus,
...
pub fn parse_manga_list(base_url: String, _filters: Vec<Filter>, page: i32) -> Result<MangaPageResult> {
    ...
    if !title.is_empty() {
        ...
        let json = request
            ...
            .body(body_data.as_bytes()).json().as_object()?;
        ...
        parse_manga_listing(base_url.clone(), String::from(format!("{}/latest-releases/{}", BASE_URL, page)), String::from("Latest"))
    }
    else {
    let mut mangas: Vec<Manga> = Vec::new();
        ...
        let has_more = if last_page.contains("»"){ true } else { false };
        Ok(MangaPageResult {
            manga: mangas,
            has_more,
        })
...
pub fn parse_manga_listing(base_url: String, url: String, list_type: String) -> Result<MangaPageResult> {
    if list_type == "Hot" {
        let mut mangas: Vec<Manga> = Vec::new();
        let html = Request::new(url.clone().as_str(), HttpMethod::Get).html();
        for manga in html.select("#manga-hot-updates .item").array() {
            ...
            let id = get_manga_id(manga_node.select("a").attr("href").read());
            let cover = base_url.clone() + manga_node.select("img").attr("src").read().as_str().trim();
            mangas.push(Manga {
                id,
                cover,
                title,
        ...
        for manga in html.select(".latest-updates .poster.poster-xs").array() {
            ...
            let title = manga_node.select("h2").text().read();
            let id = get_manga_id(manga_node.select("a").attr("href").read());
            let cover = base_url.clone() + manga_node.select("img").attr("data-src").read().as_str();
            mangas.push(Manga {
                id,
                cover,
                title,
        ...
        for manga in html.select(".clearfix.mb-0 li").array() {
            ...
            let title = manga_node.select("h2").text().read();
            let id = get_manga_id(manga_node.select("a").attr("href").read());
            let cover = base_url.clone() + manga_node.select("img").attr("data-src").read().as_str().trim();
            mangas.push(Manga {
                id,
                cover,
                title,
...
pub fn parse_manga_details(base_url: String, raw_id: String) -> Result<Manga> {
    ...
    let description = html.select(".series-summary-wrapper span").first().text().read();
    let status = manga_status(html.select("div.series-genres").text().read().to_uppercase());
    ...
    let manga_type = html.select("table td:eq(0) div").text().read().replace("Type", "").replace(" ", "");
...
pub fn parse_chapter_list(_base_url: String, _date_format: String,
    id: String,
) -> Result<Vec<Chapter>> {
