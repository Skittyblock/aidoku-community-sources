...
pub fn get_lang_code() -> Option<String> {
	if let Ok(languages) = defaults_get("languages").ok()?.as_array() {
...
pub fn get_manga_url(manga_id: &str, base_url: String) -> String {
	...
	// } else if base_url.contains("m.webtoons.com") {
	// 	format!("{}/x/x/list?title_no={}", base_url, manga_id)
	} else {
