...
use aidoku::{
    Chapter, error::Result, Manga,
    MangaContentRating, MangaStatus, MangaViewer, Page, prelude::*, std::html::Node, std::String, std::Vec,
};
...
pub fn parse_manga_list(html: Node) -> Vec<Manga> {
    ...
    for page in html.select(".ranking_lst.popular").array().next().unwrap().as_node().select("ul > li").array() {
        ...
        let image = obj.select("img").attr("src").read().replace("a92", "crop540_540");
        let title = obj.select(".subj").first().text().read().trim().to_string();
        let author = obj.select(".author").first().text().read().trim().to_string();
...
pub fn parse_search(html: &Node, challenge: bool) -> Vec<Manga> {
    let mut result: Vec<Manga> = Vec::new();
    let selector_class = if challenge { ".challenge_lst" } else { ".card_lst" };

    for page in html.select(selector_class).select("li").array() {
        let obj = page.as_node();

        let image = obj.select("img").attr("src").read();
        let title = obj.select(".subj").first().text().read().trim().to_string();
        let author = obj.select(".author").first().text().read().trim().to_string();
        ...
        let url_prefix = if challenge { "challenge".to_string() } else { genre.replace(" ", "-").to_lowercase() };
        let id_num = substr_after(obj.select("a").attr("href").read().as_str(), "titleNo=").to_string();

        let url = format!("{}/en/{}/{}/list?title_no={}", BASE_URL, url_prefix, url_title, id_num);
        let id = substr_after(&url, "webtoons.com/en/").to_string();
...
pub fn parse_manga(obj: Node, id: String) -> Result<Manga> {
    ...
    let mut cover = obj.select(".background_pic").select("img").attr("src").read();
    if cover.is_empty() { cover = obj.select(".detail_chal_pic").select("img").attr("src").read(); }
    let author = obj.select(".author").first().text().read().trim().to_string();
...
pub fn get_chapter_list(obj: Node, manga_id: String) -> Result<Vec<Chapter>> {
    ...
    for chapter in obj.select("#_episodeList").select("li").array() {
        ...
        let title = obj.select(".sub_title span").text().read().trim().to_string()
            .replace("&amp;", "&")
            .replace("&quot;", "\"")
            .replace("&#039;", "'")
            .replace("&lt;", "<")
