...
fn get_manga_list(filters: Vec<Filter>, page: i32) -> Result<MangaPageResult> {
	...
	let html = parser::request_get(&mut url).html()?;
	if url.contains("https://www.colamanhua.com/show") {
...
fn get_manga_details(manga_id: String) -> Result<Manga> {
	let mut url = format!("{}/manga-{}/", BASE_URL, manga_id);
...
fn get_chapter_list(manga_id: String) -> Result<Vec<Chapter>> {
	let mut url = format!("{}/manga-{}/", BASE_URL, manga_id);
	parser::get_chapter_list(parser::request_get(&mut url).html()?)
}

#[get_page_list]
fn get_page_list(manga_id: String, chapter_id: String) -> Result<Vec<Page>> {
	let mut url = format!("{}/manga-{}/1/{}.html", BASE_URL, manga_id, chapter_id);
