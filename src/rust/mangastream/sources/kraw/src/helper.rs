...
pub fn get_listing_url(source: &MangaStreamSource, listing_name: String, page: i32) -> String {
	...
	let list_type = if listing_name == source.listing[0] && code == "ko" {
		"genre%5B%5D=263&type=manhwa&order=update"
	} else if listing_name == source.listing[1] && code == "ko" {
		"genre%5B%5D=263&type=manhwa&order=popular"
	} else if listing_name == source.listing[2] && code == "ko" {
		"genre%5B%5D=263&type=manhwa&order=latest"
	} else {
		&source.base_url
	};
	let url = match page {
	...
	println!("{}", url);
	url
	/*if url != source.base_url && code == "zh" {
		parse_manga_listing_zh(
			source, url.clone())
	} else {
		url
	}*/
