use aidoku::{
	error::Result, std::String, std::{Vec, json::parse}, std::net::Request, std::net::HttpMethod,
	Filter, FilterType, Listing, Manga, MangaPageResult, Page, MangaStatus, MangaContentRating, MangaViewer, Chapter, DeepLink, prelude::println
};
		...
		match filter.kind {
			...
			FilterType::Genre => {
				match filter.value.as_int().unwrap_or(-1) {
					1 => included_tags.push(get_tag_id(base_url.clone(), filter.name)),
					_ => continue,
				}
			},
			FilterType::Select => {
...
pub fn parse_manga_details(id: String) -> Result<Manga> {
    ...
    for node in html.select("span.mgen a").array() {
        ...
        if category.clone().as_str() == "Mature" || category.clone().as_str() == "Ecchi" || category.clone().as_str() == "Hentai" {
