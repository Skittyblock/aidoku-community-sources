...
pub fn get_manga_details(base_url: String, id: String) -> Result<Manga> {
	let url = get_manga_url(base_url, id.clone());
	let html = Request::new(url.as_str(), HttpMethod::Get).html();
...
pub fn get_chapter_list(base_url: String, id: String) -> Result<Vec<Chapter>> {
...
pub fn get_page_list(base_url: String, id: String) -> Result<Vec<Page>> {
...
pub fn handle_url(base_url: String, url: String) -> Result<DeepLink> {
	...
	if url.starts_with("manga") {
		...
		let manga = get_manga_details(base_url, String::from(manga_id))?;
