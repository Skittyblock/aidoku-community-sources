...
pub fn get_manga_list(filters: Vec<Filter>, page: i32) -> Result<MangaPageResult> {
	let search_url = parser::get_filter_url(&filters, page).ok_or(WNode::PARSING_ERROR)?;
	let html = wrappers::get_html(&search_url)?;
	let mangas = parser::parse_search_results(&html).ok_or(WNode::PARSING_ERROR)?;
...
pub fn get_manga_listing(listing: Listing, _page: i32) -> Result<MangaPageResult> {
	...
	let mangas = parser::parse_lising(&html, listing).ok_or(WNode::PARSING_ERROR)?;
...
pub fn get_page_list(manga_id: String, chapter_id: String) -> Result<Vec<Page>> {
	...
	let html = wrappers::get_html(&url)?;
	parser::get_page_list(&html).ok_or(WNode::PARSING_ERROR)
}

#[handle_url]
pub fn handle_url(url: String) -> Result<DeepLink> {
	let manga_id = helpers::get_manga_id(&url).ok_or(AidokuError {
